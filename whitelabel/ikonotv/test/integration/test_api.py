...
from whitelabel.ikonotv.api import IkonotvUserEndpoint, IkonotvPieceEndpoint
from whitelabel.ikonotv.test.util import APIUtilWhitelabelIkonotv


class IkonotvUserEndpointTest(TestCase,
                              APIUtilUsers,
                              APIUtilDigitalWork,
                              APIUtilThumbnail,
                              APIUtilPiece,
                              APIUtilWhitelabelIkonotv,
    ...
    def setUp(self):
        ...
        self.market = self.create_whitelabel_ikonotv(self.user_admin)
    ...
    def test_list_web(self):
        ...
        view = IkonotvUserEndpoint.as_view({'get': 'list'})

        url = '/api/whitelabel/ikonotv/users/'
    ...
    def test_acl_web_user(self):
        ...
        # false because has no contractagreement yet
        self.assertFalse(user_acl['acl_wallet_submit'])

        self.assertFalse(user_acl['acl_create_contractagreement'])
        self.assertFalse(user_acl['acl_edit_private_contract'])
        self.assertFalse(user_acl['acl_view_settings_contract'])

        # give user1 a contractagreement
        self.create_contractagreement(self.user_admin, self.user1)
        response = view(request)
        web_user = response.data['users'][0]
        user_acl = web_user['acl']
        self.assertTrue(user_acl['acl_create_piece'])
    ...
    def test_acl_admin_user(self):
        ...
        self.assertTrue(user_acl['acl_create_contractagreement'])
        self.assertTrue(user_acl['acl_edit_private_contract'])
        self.assertTrue(user_acl['acl_view_settings_contract'])


class IkonotvPieceEndpointTest(TestCase,
                               APIUtilUsers,
                               APIUtilDigitalWork,
                               APIUtilThumbnail,
                               APIUtilPiece,
    ...
    def test_list_web_user(self):
        ...
        He should not have access to the other pieces in the db.
        """
        view = IkonotvPieceEndpoint.as_view({'get': 'list'})

        url = '/api/whitelabel/ikonotv/pieces/'
        request = self.factory.get(url)
        force_authenticate(request, user=self.user1)
    ...
    def test_acl_piece_user(self):
        ...
        He should not have access to the other pieces in the db.
        """
        view = IkonotvPieceEndpoint.as_view({'get': 'retrieve'})

        piece_id = self.piece_user1.id
        url = '/api/whitelabel/ikonotv/pieces/{}'.format(self.market.subdomain, piece_id)
        request = self.factory.get(url)
    ...
    def test_acl_piece_admin(self):
        ...
        url = '/api/whitelabel/ikonotv/pieces/{}'.format(self.market.subdomain, piece_admin.id)
        request = self.factory.get(url)
    ...
    def test_acl_piece_user_submit(self):
        ...
        url = '/api/whitelabel/ikonotv/pieces/{}'.format(piece.id)
        request = self.factory.get(url)
